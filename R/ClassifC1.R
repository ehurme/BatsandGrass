...
#setwd("/Volumes/LACIE SHARE/Master/MasterArbeit/Auswertung/Tadarida/Tadarida-C-master/tadaridaC_src/other_inputs")

#get arguments from the command line
#args <- commandArgs(trailingOnly = TRUE)
#uncomment the following line if you prefer to do not use R in command line
args="J:/test/merged_treated_files_HG/txt"
args[2]="ClassifEsp_tabase3HF_Baden-̈Wuerttemberg_2022-04-27.learner"
...
args[9]=8409 #end number number
obslist=talistot
FIO=FITA
#obslist=list.files(args[1],pattern=".ta$",full.names=T,recursive=T) # 1e-4 sec/files
#args[15]="ClassifEsp_tabase3HF_Baden-Württemberg_2021-04-20.learner"
...
if (mean(testReduc)>0.5) #when a reduction of variables is necessary (=low frequency case)
{
  ...
  for (i in 4:274)
  {
    ...
    test2=findInterval(temp,cuts,left.open=TRUE)
